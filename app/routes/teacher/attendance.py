...
def attendance():
    ...
    if not session.get("teacher"): return redirect(url_for("login.login"))
    ...
    try:
        ...
        assigned_department_ids = {assignment.department_id for assignment in assignments if assignment.department_id is not None}
        ...
        assigned_semesters = sorted({assignment.semester for assignment in assignments if assignment.semester is not None})
        ...
        if request.method == "POST":
            ...
            if department_value: selected_department_id = int(department_value)
            if semester_value: selected_semester = int(semester_value)
            if sessions_value: selected_sessions = sessions_value

        assigned_students_query = AddStudentInfo.query.filter_by(teacher_id=teacher_id)

        
        if selected_department_id is not None: assigned_students_query = assigned_students_query.filter_by(department_id=selected_department_id)
        if selected_semester is not None: assigned_students_query = assigned_students_query.filter_by(semester=selected_semester)
        
        assigned_students = assigned_students_query.all()
        session_values = sorted({student.sessions for student in assigned_students if student.sessions})
        form.sessions.choices = [(sessions, sessions) for sessions in session_values]
        subject_assignments = TeacherAssignment.query.filter_by(teacher_id=teacher_id)
        
        if selected_department_id is not None: subject_assignments = subject_assignments.filter_by(department_id=selected_department_id)
        if selected_semester is not None: subject_assignments = subject_assignments.filter_by(semester=selected_semester)
        ...
        if selected_sessions is not None: assigned_students = [student for student in assigned_students if student.sessions == selected_sessions]
        ...
        if form.validate_on_submit():
            ...
            if not assignment:
                ...
                return render_template("teacher/attendance.html", form=form, students=[], saved_attendance={})
            student_data = AddStudentInfo.query.filter_by(teacher_id=teacher_id, department_id=selected_department_id, semester=selected_semester, sessions=selected_sessions, group=selected_group).order_by(AddStudentInfo.student_roll).all()
